...
function calculatorStrings(express) {
  //先去掉表达式中的所有空格
  ...
  for (let i = 0; i < express.length; i++) {
    if (express[i] == "(") {
      ...
      if (symbolStack.length === 0) symbolStack.push(itemSy); //如果运算符的栈为空,直接将运算符入栈
      ...
      else if (getLea(itemSy) > getLea(symbolStack[symbolStack.length - 1])) {
        //如果当前运算符大于栈顶运算符的优先级，直接入栈
        symbolStack.push(itemSy);
      } else {
        //如果当前运算符小于等于栈顶运算符,则弹出栈顶运算符加入到后缀表达式中
        houExpress.push(symbolStack.pop());
        ...
        if (symbolStack.length == 0 || symbolStack[symbolStack.length - 1] == "(" || getLea(itemSy) > getLea(symbolStack[symbolStack.length - 1])) {
          ...
          //说明出栈后的栈顶运算符优先级仍然大于等于当前于运算符，于是通过循环继续查找判断
          var max = symbolStack.length - 1; //获取最大下标
          while (getLea(itemSy) <= getLea(symbolStack[max])) {
            //继续在运算符栈里面查找：新的栈顶运算符是否还是大于等于当前运算符，如果大于等于则继续出栈
            houExpress.push(symbolStack.pop());
            if (max > 0) max--;
            else {
              symbolStack.push(itemSy);
  ...
  //后缀表达式读完，当前栈顶元素即为结果
  return String(numStack[0]); //返回结果
}

export default calculatorStrings;
